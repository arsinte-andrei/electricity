...
#include "ui/uiAtpCompanyPoints.h"
#include "AtpSqlQuery.h"

#include <QMessageBox>

AtpCompanyPoints::AtpCompanyPoints(QWidget *parent) : QWidget(parent), ui(new Ui::AtpCompanyPoints){
	ui->setupUi(this);
...
AtpCompanyPoints::~AtpCompanyPoints(){
	delete ui;
}

void AtpCompanyPoints::updateDb() {
	ui->enableAndDisable("reset");

	model->setTable("comp_point_price_t");
...
void AtpCompanyPoints::onTablePointsClicked(const QModelIndex &index){
	int rand = index.row();
...
void AtpCompanyPoints::onEditSearchTextChanged(const QString &arg1){
...
void AtpCompanyPoints::onButtonCancelClicked(){
	if (seEediteaza){
			ui->enableAndDisable("reset");
			resetVars();
		} else {
...
void AtpCompanyPoints::onButtonEditClicked(){
	QString buttonName;
...
void AtpCompanyPoints::onButtonDeleteClicked(){
	QMessageBox msgBox;
...
void AtpCompanyPoints::onButtonNewClicked(){
	QString buttonName;
...
void AtpCompanyPoints::onButtonOkClicked(){
	if (seEediteaza){
...
void AtpCompanyPoints::resetVars(){
	seEediteaza = false;
	eNou = false;
	eVechi = false;
	punctId = 0;
